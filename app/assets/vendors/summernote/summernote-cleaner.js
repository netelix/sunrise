    ...
    'cleaner': function (context) {
      ...
      var cleanText = function (txt, nlO) {
        ...
        if (options.cleaner.keepHtml) {
          ...
          rwp = options.cleaner.replaceWithP;
          for (var ii = 0; ii < rwp.length; ii++ ) {
            var tag = rwp[ii];
            out = out.replace(new RegExp("<"+ tag +"[^>]*>?","g"), '<p>').replace(new RegExp("<\/"+ tag +">","g"), '</p>');
          }

          rws = options.cleaner.replaceWithSpan;
          for (var ii = 0; ii < rws.length; ii++ ) {
            var tag = rws[ii];
            out = out.replace(new RegExp("<"+ tag +"[^>]*>?","g"), '<span>').replace(new RegExp("<\/"+ this +">","g"), '</span>');
          }
        }
        return out;
      };
          ...
          if (options.cleaner.action == 'both' || options.cleaner.action == 'paste') {
            ...
            if (text) {
              if (msie || ffox)
                ...
                $note.summernote('pasteHTML', cleanText(text, options.cleaner.newline));
              if (options.cleaner.showMessage && $editor.find('.note-status-output').length > 0 )
